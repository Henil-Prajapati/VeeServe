...
app.get("/", function (req, res) {
  serviceCategory.find({ isActive: true }).exec((err, foundCategories) => {
    if (err) console.log(err);
      ...
      feedback
        ...
        .populate("userId")
        .exec((err, feedbacks) => {
          // Check if user is logged in and has an email address
          if (req.session.isLoggedIn && req.session.emailAddress) {
            const userEmail = req.session.emailAddress;

            if (req.cookies.newcart) {
              // If user has items in the cart cookie, save them to the database
              ...
              Cart.findOne({ user: req.session.userId })
                ...
                .then(function (updatedCart) {
                  // Clear the cart cookie and update the userCart variable
              ...
              // If user does not have items in the cart cookie, fetch their cart from the database
              Cart.findOne({ user: req.session.userId }).exec(
                (err, foundCart) => {
                  if (err) console.log(err);
                  else {
                    // Update the userCart global variable with the cart items
            ...
            // If user is not logged in, check if they have items in the cart cookie
            if (req.cookies.newcart) {
              ...
              // Render EJS page and pass feedbacks and user names as locals
              userCart = { items: [] };
              console.log(userCart);
              res.render("index", {
                status: 0,
...
app.get("/services", function (req, res) {
  serviceCategory.find({ isActive: true }, function (err, foundCategories) {
    if (err) {
      console.log(err);
    ...
    if (req.session.isLoggedIn) {
      Cart.findOne({ user: req.session.userId }).exec((err, foundCart) => {
        if (err) {
          console.log(err);
          // Handle the error, e.g., by sending an error response
          return res.status(500).send("Internal Server Error");
    ...
    .exec(function (err, foundServices) {
      if (err) {
        ...
        if (foundServices.length > 0) {
          ...
          // Check if the user is logged in and query the cart table to fetch the cart items
          if (req.session.isLoggedIn) {
            Cart.findOne({ user: req.session.userId }).exec(
              (err, foundCart) => {
                if (err) {
                  ...
                  // Get the service IDs in the cart
                  const cartServiceIds = foundCart.items.map((item) =>
                    item.serviceId.toString()
                  );

                  // Render the services_detail EJS page and pass the services and cart information
